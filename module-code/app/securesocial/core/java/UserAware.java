    ...
    public F.Promise<Result> call(final Http.Context ctx) {
        ...
        try {
            ...
            return F.Promise.wrap(env.authenticatorService().fromRequest(ctx._requestHeader())).flatMap(
                    new Function<Option<Authenticator<Object>>, Promise<Result>>() {
                        @Override
                        public Promise<Result> apply(Option<Authenticator<Object>> authenticatorOption) {
                            if (authenticatorOption.isDefined() && authenticatorOption.get().isValid()) {
                                Authenticator authenticator = authenticatorOption.get();
                                return F.Promise.wrap(authenticator.touch()).flatMap(new Function<Authenticator, Promise<Result>>() {
                                    @Override
                                    public Promise<Result> apply(Authenticator touched) {
                                        ctx.args.put(SecureSocial.USER_KEY, touched.user());
                                        return F.Promise.wrap(touched.touching(ctx)).flatMap(new Function<scala.runtime.BoxedUnit, Promise<Result>>() {
                                            @Override
